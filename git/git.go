...
func LsRemote(remote, remoteRef string) (string, error) {
	...
	if remoteRef == "" {
		return simpleGitWithoutLFS("ls-remote", remote)

	}
	return simpleGitWithoutLFS("ls-remote", remote, remoteRef)
}

func ResolveRef(ref string) (*Ref, error) {
	outp, err := simpleGitWithoutLFS("rev-parse", ref, "--symbolic-full-name", ref)
	if err != nil {
...
func LocalRefs() ([]*Ref, error) {
	cmd := gitWithoutLFS("show-ref", "--heads", "--tags")
...
func UpdateIndex(file string) error {
	_, err := simpleGitWithoutLFS("update-index", "-q", "--refresh", file)
	return err
}
...
func (c *gitConfig) SetSystem(key, val string) (string, error) {
	return simpleGitWithLFS("config", "--system", key, val)
}
...
func (c *gitConfig) UnsetGlobal(key string) (string, error) {
	return simpleGitWithLFS("config", "--global", "--unset", key)
}
...
func (c *gitConfig) UnsetSystem(key string) (string, error) {
	return simpleGitWithLFS("config", "--system", "--unset", key)
}
...
func (c *gitConfig) UnsetGlobalSection(key string) (string, error) {
	return simpleGitWithLFS("config", "--global", "--remove-section", key)
}
...
func (c *gitConfig) UnsetSystemSection(key string) (string, error) {
	return simpleGitWithLFS("config", "--system", "--remove-section", key)
}
...
func (c *gitConfig) UnsetLocalSection(key string) (string, error) {
	return simpleGitWithLFS("config", "--local", "--remove-section", key)
}
...
func (c *gitConfig) List() (string, error) {
	return simpleGitWithLFS("config", "-l")
}
...
func (c *gitConfig) ListFromFile(f string) (string, error) {
	return simpleGitWithLFS("config", "-l", "-f", f)
}
...
func (c *gitConfig) Version() (string, error) {
	...
	if len(c.gitVersion) == 0 {
		v, err := simpleGitWithLFS("version")
		if err != nil {
			return v, err
		}
		c.gitVersion = v
...
func gitWithoutLFS(args ...string) *subprocess.Cmd {
	return subprocess.ExecCommand("git", gitConfigNoLFS(args...)...)
}

// Invoke Git with disabled LFS filters
func simpleGitWithoutLFS(args ...string) (string, error) {
	return subprocess.SimpleExec("git", gitConfigNoLFS(args...)...)
}

func gitWithLFS(args ...string) *subprocess.Cmd {
	return subprocess.ExecCommand("git", args...)
}

func simpleGitWithLFS(args ...string) (string, error) {
...
func RecentBranches(since time.Time, includeRemoteBranches bool, onlyRemote string) ([]*Ref, error) {
	cmd := gitWithoutLFS("for-each-ref",
		`--sort=-committerdate`,
...
func GitAndRootDirs() (string, string, error) {
	cmd := gitWithoutLFS("rev-parse", "--git-dir", "--show-toplevel")
	buf := &bytes.Buffer{}
	cmd.Stderr = buf
...
func RootDir() (string, error) {
	cmd := gitWithoutLFS("rev-parse", "--show-toplevel")
	out, err := cmd.Output()
	if err != nil {
...
func GitDir() (string, error) {
	cmd := gitWithoutLFS("rev-parse", "--git-dir")
	out, err := cmd.Output()
	if err != nil {
...
func Checkout(treeish string, paths []string, force bool) error {
	...
	_, err := simpleGitWithoutLFS(args...)
	return err
}
...
func RemoteRefs(remoteName string) ([]*Ref, error) {
	var ret []*Ref
	cmd := gitWithoutLFS("ls-remote", "--heads", "--tags", "-q", remoteName)
